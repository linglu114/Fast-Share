...
/// FLP Frame Layer — 统一帧封装 (FLP v1.2 §3)
///
/// Frame 结构:
/// +--------+---------+------+-------+--------+----------+--------+----------+
/// | magic  | frameVer| type | flags | length | reserved | payload| checksum |
/// | 4B     | 1B      | 1B   | 2B    | 4B     | 4B       | N      | 4B       |
/// +--------+---------+------+-------+--------+----------+--------+----------+
class FlpFrame {
  /// Magic bytes: ASCII "FSP1" = 0x46 0x53 0x50 0x31
  static const List<int> magic = [0x46, 0x53, 0x50, 0x31];

  /// Frame 格式版本，固定 0x01
  static const int frameVersion = 0x01;

  /// Header 长度 (不含 payload 和 checksum): 16 bytes
  ...
  /// 从字节流构建 Frame
  factory FlpFrame.fromPayload({
    required int type,
    int flags = 0,
    required Uint8List payload,
  }) {
    if (payload.length > maxFrameLength) {
      throw FlpFrameException('Payload exceeds max frame length');
    }
  ...
  /// 序列化为网络字节流
  Uint8List toBytes() {
    final totalLength = headerLength + payload.length + checksumLength;
    ...
    // payload — bulk copy
    result.setAll(headerLength, payload);

    // checksum: CRC32(header + payload)
    final crc = crc32(Uint8List.sublistView(result, 0, headerLength + payload.length));
    bd.setUint32(headerLength + payload.length, crc, Endian.big);

    return result;
  }

  /// 从网络字节流解析 Frame
  static FlpFrame parse(Uint8List bytes) {
    if (bytes.length < headerLength + checksumLength) {
      throw FlpFrameException(
          'Frame too short: ${bytes.length} bytes, min ${headerLength + checksumLength}');
    }

    // Validate magic
    if (bytes[0] != 0x46 || bytes[1] != 0x53 || bytes[2] != 0x50 || bytes[3] != 0x31) {
      throw FlpFrameException('Invalid magic bytes');
    }

    // frameVer
    if (bytes[4] != frameVersion) {
      throw FlpFrameException('Unsupported frame version: ${bytes[4]}');
    }
...
/// CRC32 计算 (与 IEEE 802.3 / zlib 一致) — lookup-table, ~8x faster than bit-by-bit
...
class FlpFrameException implements Exception {
  final String message;
  const FlpFrameException(this.message);
  @override
  String toString() => 'FlpFrameException: $message';
}

// ═══════════════════════════════════════════════════════════
// Message Types (FLP v1.2 §4.1)
// ═══════════════════════════════════════════════════════════

class FlpMessageType {
  static const int hello = 0x01;
  static const int helloAck = 0x02;
  static const int ping = 0x03;
  static const int pong = 0x04;

  static const int pairRequest = 0x10;
  static const int pairConfirm = 0x11;
  static const int pairResult = 0x12;

  static const int transferOffer = 0x20;
  static const int transferAccept = 0x21;
  static const int transferReject = 0x22;
  static const int transferCancel = 0x23;
  static const int transferPause = 0x24;
  static const int transferResume = 0x25;

  static const int fileMeta = 0x30;
  static const int fileData = 0x31;
  static const int fileAck = 0x32;
...
class FlpFlags {
  ...
  static const int compressed = 1 << 1;
  static const int encrypted = 1 << 2;
